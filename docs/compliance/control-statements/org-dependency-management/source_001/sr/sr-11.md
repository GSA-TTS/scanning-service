---
x-trestle-comp-def-rules:
  org-dependency-management:
    - name: sha-pins-verified
      description: 'All GitHub Actions uses: refs in the repo are SHA-pinned'
    - name: actions-sast-scanned
      description: GitHub Actions workflows are scanned for security issues via 
        centralized zizmor SAST analysis
x-trestle-add-props:
  - name: implementation-evidence
    value: "Renovate dependency management is centrally configured to pin all dependencies to exact versions ('rangeStrategy': 'pin') in scanning-service/default.json5. This supports authenticity and anti-counterfeit controls for all managed software components."
x-trestle-param-values:
  sr-11_odp.01:
  sr-11_odp.02:
  sr-11_odp.03:
x-trestle-global:
  profile:
    title: Electronic Version of NIST SP 800-53 Rev 5.1.1 Controls and SP 
      800-53A Rev 5.1.1 Assessment Procedures
    href: 
      https://raw.githubusercontent.com/usnistgov/oscal-content/refs/tags/v1.3.0/nist.gov/SP800-53/rev5/json/NIST_SP-800-53_rev5_catalog.json
  sort-id: sr-11
---

# sr-11 - \[Supply Chain Risk Management\] Component Authenticity

## Implementation Evidence

- Renovate dependency management is centrally configured to pin all dependencies to exact versions ("rangeStrategy": "pin") in scanning-service/default.json5. This supports authenticity and anti-counterfeit controls for all managed software components.

## Control Statement

- \[a.\] Develop and implement anti-counterfeit policy and procedures that include the means to detect and prevent counterfeit components from entering the system; and

- \[b.\] Report counterfeit system components to {{ insert: param, sr-11_odp.01 }}.

## Control Assessment Objective

- \[SR-11a.\]

  - \[SR-11a.[01]\] an anti-counterfeit policy is developed and implemented;
  - \[SR-11a.[02]\] anti-counterfeit procedures are developed and implemented;
  - \[SR-11a.[03]\] the anti-counterfeit procedures include the means to detect counterfeit components entering the system;
  - \[SR-11a.[04]\] the anti-counterfeit procedures include the means to prevent counterfeit components from entering the system;

- \[SR-11b.\] counterfeit system components are reported to {{ insert: param, sr-11_odp.01 }}.

## Control guidance

Sources of counterfeit components include manufacturers, developers, vendors, and contractors. Anti-counterfeiting policies and procedures support tamper resistance and provide a level of protection against the introduction of malicious code. External reporting organizations include CISA.

______________________________________________________________________

## What is the solution and how is it implemented?

<!-- For implementation status enter one of: implemented, partial, planned, alternative, not-applicable -->

<!-- Note that the list of rules under ### Rules: is read-only and changes will not be captured after assembly to JSON -->

SHA pinning prevents tag-hijacking attacks where an attacker force-pushes a malicious commit to an existing tag. The SHA-256 hash guarantees the Action code matches what was originally reviewed.

Centralized SAST analysis further detects ref-confusion and impostor commit risks — cases where ambiguous git references could resolve to unintended (potentially malicious) code. Together, SHA pinning and SAST scanning prevent counterfeit or tampered dependencies from entering the CI/CD pipeline.

### Rules:

  - sha-pins-verified
  - actions-sast-scanned

### Implementation Status: implemented

______________________________________________________________________
