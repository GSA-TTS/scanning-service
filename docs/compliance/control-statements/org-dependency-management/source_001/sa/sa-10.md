...
x-trestle-add-props:
  - name: implementation-evidence
    value: "Renovate dependency management is centrally configured to pin all dependencies to exact versions ('rangeStrategy': 'pin') in scanning-service/default.json5. This ensures developer configuration management practices are enforced for all managed dependencies."
...
- Renovate dependency management is centrally configured to pin all dependencies to exact versions ("rangeStrategy": "pin") in scanning-service/default.json5. This ensures developer configuration management practices are enforced for all managed dependencies.
...
The org mandates that all CI/CD pipeline dependencies be tracked, versioned, and updated through a controlled process (Renovate + PR review). The lint check verifies compliance. SHA-pinned refs and lockfiles provide a complete manifest of external dependencies.
